...
def is_valid_mac(mac):
    return bool(re.fullmatch(r'^([0-9A-Fa-f]{2}[:-]){5}[0-9A-Fa-f]{2}$', mac) or re.fullmatch(r'^[0-9A-Fa-f]{12}$', mac))

def get_mac_info(mac):
    if not is_valid_mac(mac):
        return f"MAC-адрес: {mac}\nФормат MAC-адреса некорректен."
    ...
    try:
        ...
        if response.status_code == 200 and response.text.strip():
            return f"MAC-адрес: {mac}\nПроизводитель устройства: {response.text.strip()}"
        else:
            return f"MAC-адрес: {mac}\nПроизводитель не найден или сервер не отвечает."
    except Exception as e:
        return f"MAC-адрес: {mac}\nОшибка при определении производителя: {e}"
...
def ask_mac_address(ip, root):
    mac_dialog = tk.Toplevel()
    mac_dialog.title("Дополнительная информация (необязательно)")
    center_window(mac_dialog, 400, 150)

    ttk.Label(mac_dialog, text="Введите MAC-адрес (или нажмите 'Пропустить')", font=("Arial", 10)).pack(pady=(15, 5))
    mac_entry = ttk.Entry(mac_dialog, font=("Arial", 11), width=40)
    mac_entry.pack(pady=5)
    mac_entry.focus()

    btn_frame = ttk.Frame(mac_dialog)
    btn_frame.pack(pady=15)

    def proceed_with_mac():
        ...
        mac_info = get_mac_info(mac) if mac else "MAC-адрес не указан."
    ...
    ttk.Button(btn_frame, text="Продолжить", command=proceed_with_mac).grid(row=0, column=0, padx=10)
    ttk.Button(btn_frame, text="Пропустить", command=lambda: [mac_entry.delete(0, tk.END), proceed_with_mac()]).grid(row=0, column=1, padx=10)
...
def create_welcome_window():
    ...
    text_label = ttk.Label(
        ...
        text=(
            ...
            "Дополнительно вы можете указать MAC-адрес — это необязательно."
        ),
        wraplength=330,
        font=("Arial", 10),
        justify='center',
